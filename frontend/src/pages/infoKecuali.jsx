...
export const DIK = (props) => {
  const [listKecuali, setListKecuali] = useState([]); 
  ...
  useEffect(() => {
    const fetchKecualiData = async () => {
      try {
        ...
        if (result.success) {
          const kecualiItems = result.data.filter(item => item.kategori === 'dikecualikan');
          const sortedItems = kecualiItems.sort((a, b) => b.tahun - a.tahun);
          setListKecuali(sortedItems);
        } else {
  ...
  const currentCards = listKecuali.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(listKecuali.length / itemsPerPage);
    ...
    <Box sx={{ bgcolor: "#f8f9fa", minHeight: "50vh" }}>
      ...
      >
        <Container maxWidth="lg">
          ...
          <Typography variant="h5" sx={{ opacity: 0.9, color: 'white', fontWeight: "normal", maxWidth: "850px", fontSize: "1.3rem", textTransform: "none", lineHeight: 1.6 }}>
            Portal Informasi Dikecualikan Dinas Tenaga Kerja dan Transmigrasi Provinsi Jawa Tengah. Temukan data, laporan, dan dokumen terkait informasi yang dikecualikan dari publikasi sesuai peraturan perundang-undangan.
      ...
      <Container maxWidth="xl" sx={{ mt: -10, pb: 10, position: 'relative', zIndex: 10, display: 'flex', justifyContent: 'center' }}>
        <Card sx={{ width: '100%', borderRadius: 6, p: { xs: 3, md: 6 }, boxShadow: "0 15px 40px rgba(0,0,0,0.12)", position: 'relative'}}>
          ...
          {!loading && !error && listKecuali.length === 0 && (
            <Typography variant="body1" color="textSecondary" textAlign="center" sx={{ py: 3 }}>
              Belum ada data Informasi Dikecualikan saat ini.
            </Typography>
          )}
